...
namespace oclCrypto
{
	class Camellia
	{
		public static void Encrypt (byte[] key, byte[] input, byte[] output)
		{
			byte[] temp = new byte[16];
			uint[] temp32 = new uint[4];
			uint[] keyTable;
			GenerateKeyTable (key, out keyTable);
			for (int pos = 0; pos < input.Length; pos += 16) {
				for (int i = 0; i < 16; i++)
					temp[i] = input[pos + i];

				temp32[0] = BitConverter.ToUInt32 (temp, 0);
				temp32[1] = BitConverter.ToUInt32 (temp, 4);
				temp32[2] = BitConverter.ToUInt32 (temp, 8);
				temp32[3] = BitConverter.ToUInt32 (temp, 12);
				Encrypt (temp32, keyTable);
				BitConverter.GetBytes (temp32[0]).CopyTo (temp, 0);
				BitConverter.GetBytes (temp32[1]).CopyTo (temp, 4);
				BitConverter.GetBytes (temp32[2]).CopyTo (temp, 8);
				BitConverter.GetBytes (temp32[3]).CopyTo (temp, 12);
		...
		public static void GenerateKeyTable (byte[] key, out uint[] keyTable)
		{
			uint[] key32 = new uint[key.Length / 4];
			for (int i = 0; i < key.Length; i += 4)
				key32[i >> 2] = BitConverter.ToUInt32 (key, i);
			GenerateKeyTable (key32, out keyTable);
		}

		public static void GenerateKeyTable (uint[] key, out uint[] keyTable)
		{
			uint s1, s2, U, D;
			uint[] t = new uint[16];
			keyTable = new uint[272 / 4];
			switch (key.Length * 4) {
				case 16: /* 128bit */
					t[0] = key[0]; t[1] = key[1];
					t[2] = key[2]; t[3] = key[3];
			...
			if (key.Length * 4 == 16) {
				keyTable[0] = t[0]; keyTable[1] = t[1];
				keyTable[2] = t[2]; keyTable[3] = t[3];
				keyTable[4] = t[8]; keyTable[5] = t[9];
				keyTable[6] = t[10]; keyTable[7] = t[11];
				...
				keyTable[0] = t[0]; keyTable[1] = t[1]; keyTable[2] = t[2]; keyTable[3] = t[3];
				keyTable[4] = t[12]; keyTable[5] = t[13]; keyTable[6] = t[14]; keyTable[7] = t[15];
		...
		static void RotBlock (uint[] x, int x_offset, int n, uint[] y, int y_offset)
		{
			...
			y[y_offset + 0] = LeftShift (x[x_offset + idx], r) | RightShift (x[x_offset + idx1], 32 - r);
			y[y_offset + 1] = LeftShift (x[x_offset + idx1], r) | RightShift (x[x_offset + idx2], 32 - r);
		}

		static void Encrypt (uint[] data, uint[] key)
		{
			uint[] sbox1 = SBOX1_1110;
			uint[] sbox2 = SBOX2_0222;
			uint[] sbox3 = SBOX3_3033;
			uint[] sbox4 = SBOX4_4404;
			uint x0 = data[0] ^ key[0];
			uint x1 = data[1] ^ key[1];
			uint x2 = data[2] ^ key[2];
			uint x3 = data[3] ^ key[3];
			int ki = 0;
			const int flayerLimit = 2;
