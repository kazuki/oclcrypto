...
void subBytes (__local uint4 *state, int offset);
void mixColumns (__local uint4 *state);
void shiftRows (__local uint4 *state, int id);
void round_ (__local uint4 *state, int offset, int id, const __constant uint4 *key, int key_offset);

__kernel
void encrypt (__global uint4 *global_state,
				  const __global uint4 *key,
              __local uint4 *state
)
{
	int local_id = get_local_id (0);
	int id = local_id & (PARALLELS - 1);
	int offset = id * (SBL / PARALLELS);
	global_state += (get_global_id (0) / PARALLELS) * SBL;
	state += (local_id / PARALLELS) * SBL;

	for (uint i = 0; i < 8; i ++)
		state[offset + i] = global_state[offset + i] ^ key[offset + i];
	...
	for (uint round = 1; round < 10; round ++) {
		...
		barrier (CLK_LOCAL_MEM_FENCE);
		for (uint i = 0; i < 8; i ++)
			state[offset + i] ^= key[offset + round * 32 + i];
	}

	subBytes (state, offset);
	shiftRows (state, id);
	for (uint i = 0; i < 8; i ++)
		global_state[offset + i] = state[offset + i] ^ key[offset + 320 + i];
}
